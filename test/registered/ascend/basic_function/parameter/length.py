...
class TestRequestLengthValidation(CustomTestCase):
    ...
    def test_longer_max_tokens_validation(self):
        client = openai.Client(api_key=self.api_key, base_url=f"{self.base_url}/v1")
        # 替换：不再使用文本，而是直接传入token id列表（示例使用hello对应的token id，可根据实际模型调整）
        # 注：不同模型的token id映射不同，这里仅为示例，你需要替换为实际有效的token id
        token_ids = [15496]  # "hello" 对应的GPT-3.5/4常见token id
        with self.assertRaises(openai.BadRequestError) as cm:
            client.chat.completions.create(
                model=self.model,
                messages=[
                    {
                        "role": "user",
                        "content": [{"type": "text", "token_ids": token_ids}]  # 关键：使用token_ids结构化传入
                    },
                ],
                temperature=0,
                max_tokens=1001,  # 超过限制的数值，用于触发验证
            )
